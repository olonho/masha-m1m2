...
# Scientific Rationale and Interaction Model
...
The diagram below illustrates the mechanistic model: M1-like macrophages DRIVE dormancy entry via IFN-gamma/p38/STAT1 signaling, M2-like macrophages MAINTAIN dormancy via TGF-beta2 and gap junctional intercellular communication (GJIC), and M2-like macrophages also TRIGGER exit through MMP-mediated ECM remodeling and angiogenic switching.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{diagrams/diagram5_interaction.png}
\caption{Mechanistic model of M1-driven dormancy entry (IFN-gamma/p38/STAT1), M2-driven maintenance (TGF-beta2/GJIC), and M2-triggered exit (MMPs/VEGF). DTC markers: NR2F1, p27, p-p38/p-ERK ratio.}
\end{figure}
...
# Model System and Dormancy Timeline
...
## Intrasplenic Injection Timeline

The timeline below traces DTC fate from the moment of intrasplenic injection through seeding, dormancy entry, maintenance, and exit. On Day 0, 1x10(3) MC38-GFP-Luc-Fucci2a cells are injected into the spleen. Over Days 1-3, DTCs transit the portal vein and lodge in liver sinusoids. During Days 3-14, M1-like macrophage encounters drive ~65% of DTCs into dormancy (NR2F1+, Ki67-). M2-like macrophages then dominate the maintenance phase (Days 14-84). Spontaneous exit begins variably at Weeks 8-12.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{diagrams/diagram2_timeline.png}
\caption{Intrasplenic injection and dormancy timeline. Surgery (Day 0) through seeding (Days 1-3), entry (Days 3-14), maintenance (Days 14-84), and exit (Weeks 8-12). Color-coded by phase.}
\end{figure}
...
The project follows a phased design: Phase 0 establishes reagents and dose-response data, Phase I confirms that macrophages are required (Arm A), Phase II quantifies M1 and M2 contributions separately (Arms B and C in parallel), Phase III tests the temporal switch hypothesis (Arm D), and Phase IV integrates all data. Decision gates at each phase determine whether to proceed or revise the model.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{diagrams/diagram1_workflow.png}
\caption{Overall project workflow from Phase 0 (cell engineering, mouse breeding, dose pilot) through Phase IV (analysis). Diamond decision node at bottom shows four possible outcome scenarios.}
\end{figure}

\newpage

# Experimental Design: Four Arms

## Overview

The experiment consists of four arms, each testing a specific aspect of the M1/M2 contribution model. The diagram below summarizes all treatment groups, mouse numbers, and the research question each arm addresses.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{diagrams/diagram7_four_arms.png}
\caption{Four experimental arms: (A) Total macrophage depletion tests whether macrophages are required; (B) M1 impairment tests dormancy entry; (C) M2 impairment tests dormancy maintenance; (D) Temporal switch tests M2-to-M1 transition. n=330 mice total.}
\end{figure}
...
# Readout Hierarchy and Tissue Processing

## Three-Tier Readout System

Readouts are organized in three tiers: primary (mandatory for all mice), secondary (subset), and tertiary (dedicated cohort). All tiers feed into a unified computational analysis pipeline producing four quantitative metrics: Dormancy Entry Rate (DER), Dormancy Maintenance Duration (DMD), Dormancy Exit Rate (DXR), and DTC Clearance Rate (DCR).

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{diagrams/diagram4_readouts.png}
\caption{Three-tier readout hierarchy. Primary: BLI, IF, flow cytometry, gross pathology. Secondary: p-ERK/p-p38, cytokines, immune infiltration, Fucci2a. Tertiary: scRNA-seq, spatial transcriptomics, EM, SHG imaging. All feed into computational analysis.}
\end{figure}

## Tissue Processing Pipeline

At each sacrifice timepoint (Day 7, 21, 42, 84), the liver is divided into four portions for parallel processing. The pipeline below shows how each portion is processed for immunofluorescence, flow cytometry, scRNA-seq, or cytokine analysis, with all data streams integrating into a per-mouse quantitative report.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{diagrams/diagram6_tissue_pipeline.png}
\caption{Tissue processing pipeline. Left lobe: IF (dormancy fraction, proximity analysis). Median lobe: flow cytometry (M1:M2 ratio, KC vs MoM). Right lobe: scRNA-seq (10x Chromium + CellPhoneDB). Caudate lobe: Luminex cytokines. All streams integrate into per-mouse quantitative report.}
\end{figure}
...
The decision tree below shows how experimental results will validate or refute the Temporal Switch Model. The central question is whether both M1 and M2 contribute to dormancy regulation. If confirmed (Scenario 1), the Arm D5/D4 anti-PD-1 combination strategy is tested as a clinical translation pathway. If only one phenotype matters, the model is revised accordingly.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{diagrams/diagram3_decision_tree.png}
\caption{Expected outcomes decision tree. Four scenarios based on Arm A-D results: (1) Both M1 and M2 contribute (confirms model, proceed to therapy); (2) Only M1 (focus on IFN-gamma/p38); (3) Only M2 (focus on TGF-beta2/GJIC); (4) Neither alone (non-specific contribution). Green = confirmed model; orange/red = model revision required.}
\end{figure}
